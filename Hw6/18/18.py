...
if __name__ == '__main__':
    ...
    test_X,test_y = readData("../test.dat")
    predict_y = np.zeros(test_X.shape[0])
    EoutG = np.zeros(T)
    for t in range(T):
        if sign[t]==1:
            predict_y = predict_y + alpha[t]*np.where(test_X[:,index[t]] > theta[t],1,-1)
        else:
            predict_y = predict_y + alpha[t]*np.where(test_X[:,index[t]] < theta[t],1,-1)
        EoutG[t] = sum(np.where(predict_y > 0, 1, -1) != test_y)/float(test_X.shape[0])
        print ('Eo_G%d:%.4f\t' % ((t+1),EoutG[t])),

    print "Q18=>Eout(G):",EoutG[T-1]
