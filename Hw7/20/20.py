...
if __name__ == '__main__':
    ...
    X_test,y_test = readData('../hw7_test.dat')
    T = 30000
    trees = []
    ...
    number_data = X_test.shape[0]
    predict_y = np.zeros(number_data)
    for tree in trees:
        predict_y += predict_ALLX(tree,X_test)
        tmpy = np.where( predict_y > 0, 1 , -1)

        error_rate += sum(np.where( tmpy != y_test,1,0))/float(number_data)
