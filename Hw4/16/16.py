...
print 'min Etrain: ',min_Etrain
W = W_rigde_regression(X_train[:split], Y_train[:split],pow(10, target_lambda))
Eout = calculate_Error(W, X_test, Y_test)
print 'Eout for g_lambda: ',Eout

pl.bar(dic_16.keys(),dic_16.values())
pl.xlim([min(dic_16.keys()), max(dic_16.keys())])
pl.ylim(0, (max(dic_16.values())))
pl.xlabel('lambda')
pl.ylabel('Etrain')
pl.title('16')
pl.savefig('16')
